...
image_dataset_dir = 'kitti/robok_image_dataset_608/'
class_list = [
...
def preprocess_dataset(dataset):
    ...
    for img_idx, rgb_map, target in dataset.getitem():
        ...
        for i in range(target.shape[0]):
            if target[i].sum() == 0:
                break
            with open(image_dataset_dir + "labels/{}.txt".format(img_idx), 'a+') as f:
                ...
                line = label + ' ' + '{} {} {} {} {}\n'.format(cx, cy, w, h, rz)
                f.write(line)
        cv2.imwrite(image_dataset_dir + 'images/{}.png'.format(img_idx), rgb_map[:, :, ::-1])
...
def make_train_test_list():
    ...
    name_list.sort()
    with open('config/test_image_list_608.txt', 'w') as f:
        for name in name_list[0:1000]:
            f.write(name.split('.')[0])
            f.write('\n')
    with open('config/train_image_list_608.txt', 'w') as f:
