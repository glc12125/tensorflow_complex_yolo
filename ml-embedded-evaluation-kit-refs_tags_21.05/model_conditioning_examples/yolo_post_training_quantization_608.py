...
from yolo_training_utils_608 import create_model, create_model_with_pass_through, get_data, YoloLoss
...
class LearningRateLoggingCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print('Learning rate: {:0.7f}, epoch: {}'.format(current_decayed_lr, epoch))
...
def main():
    ...
    checkpoint_filepath = 'checkpoints_leaky/complex_yolov2_{epoch:02d}-{val_loss:.2f}'
    ...
    learning_rate_callback = LearningRateLoggingCallback()

    model.fit(x=train_dataset, validation_data=test_dataset, batch_size=batch_size, epochs=total_epoch, verbose=1, shuffle=True, callbacks=[model_checkpoint_callback, learning_rate_callback])
    ...
    tflite_models_dir = pathlib.Path('./yolo_conditioned_models_608/')
